...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    val map = chars.foldLeft[Map[Char, Int]](Map.empty) { (m, ch) =>
      if (m.contains(ch)) {
        m + (ch -> (m.get(ch).get + 1))
      } else {
        m + (ch -> 1)
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    val orderList = freqs.map {
      pair =>
        new Leaf(pair._1, pair._2)
    }.sortBy(_.weight).toList
  ...
  def convert(tree: CodeTree): CodeTable = {
    tree match {
      ...
      case Fork(left, right, chars, w) => mergeCodeTables(convert(left), convert(right))
    }
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def append(bit: Bit)(pair: (Char, List[Bit])): (Char, List[Bit]) = (pair._1, bit :: pair._2)
    a.map(append(0)) ::: b.map(append(1))
  }
