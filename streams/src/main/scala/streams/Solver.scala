...
trait Solver extends GameDef {
  ...
  def done(b: Block): Boolean =
    (b.b1.row == goal.row && b.b1.col == goal.col) && (b.b2.row == goal.row && b.b2.col == goal.col)
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    val filter = neighbors.filter(b => !explored.contains(b._1))
    filter
  }
